-- Add safeguard to prevent replacing URL with itself
CREATE OR REPLACE FUNCTION public.replace_citation_tracking(
  p_article_id UUID,
  p_old_url TEXT,
  p_new_url TEXT,
  p_new_source TEXT,
...
AS $$
BEGIN
  -- Safety check: if URLs are the same, do nothing
  IF p_old_url = p_new_url THEN
    RETURN;
  END IF;
